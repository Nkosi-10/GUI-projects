...
def equal_button():
    ...
    try:
        second_num = float(second_num)  # Convert to float to handle decimals and negative numbers
        Enter.delete(0, END)
        ...
        if maths == "addition":
            ...
            if second_num != 0:
                ...
                Enter.insert(0, "Error")  # Handle division by zero
    except ValueError:
        Enter.insert(0, "Invalid Input")  # Handle invalid input

def plus_button():
    global any_num, maths
    maths = "addition"
    any_num = float(Enter.get())  # Store the first number as a float
    Enter.delete(0, END)
